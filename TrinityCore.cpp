...
bool TrinityCore::connect(const char* host, const char* user, const char* pass, const char* db) {
...
void TrinityCore::disconnect() {
    if (m_mysql && m_connected) { mysql_close(m_mysql); m_mysql = nullptr; m_connected = false; }
}
...
TrinityNeuron* TrinityCore::loadNeuronByCode(const std::string& code) {
    ...
    snprintf(query, sizeof(query),
        "SELECT id, JSON_UNQUOTE(JSON_EXTRACT(data, '$.code')), type, "
        "COALESCE(JSON_UNQUOTE(JSON_EXTRACT(data, '$.category')), ''), "
        "COALESCE(JSON_UNQUOTE(JSON_EXTRACT(data, '$.material')), 'PLYWOOD-FSF'), "
        "COALESCE(JSON_UNQUOTE(JSON_EXTRACT(data, '$.status')), ''), data "
        "FROM neuron WHERE JSON_UNQUOTE(JSON_EXTRACT(data, '$.code')) = '%s' AND is_deleted = 0 LIMIT 1",
        escapedCode);
    if (mysql_query(m_mysql, query) != 0) return nullptr;
    MYSQL_RES* result = mysql_store_result(m_mysql);
    if (!result || mysql_num_rows(result) == 0) { if (result) mysql_free_result(result); return nullptr; }
...
std::vector<TrinitySynapse> TrinityCore::loadChildren(int parentId) {
    ...
    char query[512];
    snprintf(query, sizeof(query),
        "SELECT s.id, s.parent, s.child, JSON_UNQUOTE(JSON_EXTRACT(n.data, '$.code')), s.data "
        "FROM synapse s JOIN neuron n ON s.child = n.id "
        "WHERE s.parent = %d AND n.is_deleted = 0 ORDER BY s.id", parentId);
...
std::vector<TrinityNeuron> TrinityCore::loadPendingProjects() {
    ...
    const char* query =
        ...
        "JSON_UNQUOTE(JSON_EXTRACT(data, '$.status')), data "
        "FROM neuron WHERE type = 'project' AND JSON_UNQUOTE(JSON_EXTRACT(data, '$.status')) = 'pending' "
...
bool TrinityCore::markNeuronDone(int id) {
    ...
    char query[256];
    snprintf(query, sizeof(query), "UPDATE neuron SET data = JSON_SET(data, '$.status', 'done') WHERE id = %d", id);
...
TrinityNeuron TrinityCore::parseNeuronRow(MYSQL_ROW row) {
    ...
    if (!n.code.empty()) sscanf_s(n.code.c_str(), "D.S.%d.%d.%d.%d", &n.processCode, &n.width, &n.height, &n.thickness);
    return n;
}
...
TrinitySynapse TrinityCore::parseSynapseRow(MYSQL_ROW row) {
    ...
    if (row[4]) { std::string json(row[4]); s.position = parsePosition(json); s.rotation = parseRotation(json); }
    return s;
}
...
TrinityPosition TrinityCore::parsePosition(const std::string& json) {
    ...
    if (posKey != std::string::npos) {
        ...
        if (arrStart != std::string::npos) sscanf_s(json.c_str() + arrStart, "[%lf, %lf, %lf]", &pos.x, &pos.y, &pos.z);
    }
    return pos;
}
...
TrinityRotationCompound TrinityCore::parseRotation(const std::string& json) {
    ...
    if (json[nextChar] == '[') {
        size_t rot1Start = nextChar, rot1End = json.find(']', rot1Start);
        if (rot1End != std::string::npos && compound.count < 2) {
            sscanf_s(json.c_str() + rot1Start, "[%lf, %lf, %lf, %lf]", &compound.rotations[0].x, &compound.rotations[0].y, &compound.rotations[0].z, &compound.rotations[0].angle);
            compound.count++;
        }
        size_t rot2Start = json.find('[', rot1End + 1);
        if (rot2Start != std::string::npos && compound.count < 2) {
            sscanf_s(json.c_str() + rot2Start, "[%lf, %lf, %lf, %lf]", &compound.rotations[1].x, &compound.rotations[1].y, &compound.rotations[1].z, &compound.rotations[1].angle);
            compound.count++;
        }
    } else {
        sscanf_s(json.c_str() + arrStart, "[%lf, %lf, %lf, %lf]", &compound.rotations[0].x, &compound.rotations[0].y, &compound.rotations[0].z, &compound.rotations[0].angle);
        compound.count = 1;
    }
    return compound;
}